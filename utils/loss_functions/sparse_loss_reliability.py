...
def batch_descriptor_loss_sparse_relibability(descriptors, descriptors_warped, reliability, reliability_warped, homographies, aplosser, **options):
    ...
    for i in range(batch_size):
        ...
        ap_loss.append(losses[3])
    loss, pos_loss, neg_loss, ap_loss = torch.stack(loss), torch.stack(pos_loss), torch.stack(neg_loss), torch.stack(ap_loss)
    return loss.mean(), None, pos_loss.mean(), neg_loss.mean(), ap_loss.mean()
