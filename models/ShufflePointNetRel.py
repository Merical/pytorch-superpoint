...
class ShufflePointNetRel(nn.Module):
    def __init__(self, width_mult=0.0):
        super(ShufflePointNetRel, self).__init__()
        ...
        # Reliability Head
        self.convRa = torch.nn.Conv2d(d1, 1, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x):
        ...
        reli = self.softmax_reliability(self.convRa(desc ** 2))

        dn = torch.norm(desc, p=2, dim=1)  # Compute the norm.
        desc = desc.div(torch.unsqueeze(dn, 1))  # Divide by norm to normalize.
        output = {'semi': semi, 'desc': desc, 'reli': reli}
        self.output = output
    ...
    def softmax_reliability(self, ux):
        x = F.softplus(ux)
