...
class Train_model_reliability(Train_model_frontend):
    ...
    ** note: the input, output is different from that of SuperPointFrontend
    ...
    def __init__(self, config, save_path=Path("."), device="cpu", verbose=False):
        ...
        self.aploss = APLoss(nq=20, min=0, max=1, euc=False).to(self.device)
        ...
        if self.config["model"]["dense_loss"]["enable"]:
            ...
            from utils.loss_functions.sparse_loss_reliability import batch_descriptor_loss_sparse_relibability

            self.descriptor_loss = batch_descriptor_loss_sparse_relibability
    ...
    def train_val_sample(self, sample, n_iter=0, train=False):
        ...
        if train:
            ...
            semi, coarse_desc, reli= outs["semi"], outs["desc"], outs["reli"]
            if if_warp:
                outs_warp = self.net(img_warp.to(self.device))
                semi_warp, coarse_desc_warp, reli_warp = outs_warp["semi"], outs_warp["desc"], outs_warp["reli"]
        else:
            with torch.no_grad():
                outs = self.net(img.to(self.device))
                semi, coarse_desc, reli = outs["semi"], outs["desc"], outs["reli"]
        ...
        labels_3D_max = labels_3D.max()
        labels_3D_min = labels_3D.min()
        semi_max = outs["semi"].max()
        semi_min = outs["semi"].min()
        ...
        if lambda_loss > 0:
            ...
            loss_desc, mask, positive_dist, negative_dist, loss_ap = self.descriptor_loss(
                coarse_desc,
                coarse_desc_warp,
                reli,
                reli_warp,
            ...
            loss_desc, positive_dist, negative_dist, loss_ap = ze, ze, ze, ze

        loss = loss_det + loss_det_warp
        if lambda_loss > 0:
            loss += lambda_loss * (loss_desc + loss_ap)
